(function () {
    ...
    app.controller('echeanciercontroller', ['$rootScope', '$scope', '$http', '$stateParams', 'config', function ($rootScope, $scope, $http, $stateParams,config) {
        ...
        $http({
            method: 'GET',
            url: config.url+'/Echeancier/GetEcheancierCreateView/' + $stateParams.societe + '/' + $stateParams.numcomptable
        })
        ...
        $scope.$on('EcheancierRowsSelected', function (event, rows) {
            ...
            if (rows) {
                for (var i = 0; i < rows.length; i++) {
                    $scope.EcheancierCreate.idPieces.push(rows[i].FIDPIECE);
                    $scope.EcheancierCreate.npieces.push(rows[i].FNPIECE);
                    $scope.EcheancierCreate.mtsoldes.push(rows[i].FMTSOLDE);
                    $scope.EcheancierCreate.echeancier.MontantTotal += rows[i].FMTSOLDE;
                }
            }
        });
        ...
        function AddEcheancierModel() {
            ...
            $http({
                method: 'POST',
                url: config.url+'/Echeancier/PostEcheancierCreateView',
