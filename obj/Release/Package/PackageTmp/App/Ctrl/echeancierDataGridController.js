(function () {
    ...
    app.controller('echeancierDataGridController', ['$rootScope', '$scope', '$http', 'uiGridConstants', 'gridConfService', '$stateParams', 'config', function ($rootScope, $scope, $http, uiGridConstants, gridConfService, $stateParams,config) {
        ...
        gridConfService.simpleConf$columnDefs$specificDefs($scope, config.url+'/Echeancier/GetDataObjectFacture/' + $stateParams.societe + '/' + $stateParams.numcomptable + '/', 60, defs);


        //send an event through the application scope
        $scope.rowsSelected = function (rows) {
            $rootScope.$broadcast('EcheancierRowsSelected', rows);
        }
