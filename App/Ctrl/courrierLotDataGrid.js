(function () {
    ...
    app.controller('courrierLotDataGrid', ['$rootScope', '$scope', '$http', 'uiGridConstants', 'gridConfService', '$window', 'listClientService', '$location', 'config', 'cookieService', function ($rootScope, $scope, $http, uiGridConstants, gridConfService, $window, listClientService, $location, config, cookieService) {
        $scope.gridMode = 'COURRIERLOT';
        function cellTemplate() {
            return '<div class="personal-drop dropdown"><a class="btn-burger"><span>&nbsp;</span></a><ul class="drop in-array"><li><a ui-sref="afficheRelance({id:row.entity.FILTER})">Voir le courrier</a></li><li><a href="" ng-click="grid.appScope.ClientRowClick(row)">Voir la fiche client</a></li><li><a href="" ng-click="grid.appScope.ValidRowClick(row)">Traiter cette relance</a></li></ul></div>';
        }
        ...
        $scope.refreshGrid = function () {
            gridConfService.simpleConf$columnDefs$specificDefs($scope, config.url + '/CourrierLot/GetDataObject/' + $scope.userFilter + '/' + $scope.SelectedType + '|' + $scope.SelectedSociete + '|' + $scope.SelectedScenario + '|' + $scope.SelectedNiveau + '|' + $scope.Multisearch + '/', 180, defs);
         };
        ...
        /* ------------ Events -------------*/
        $scope.$on('CourrierLotFilterClick', function (event, SelectedType, SelectedSociete, SelectedScenario, SelectedNiveau, Multisearch) {
        ...
        $scope.rowsSelected = function (rows) {
            ...
            $scope.ids = ids;
            $rootScope.$broadcast('CourrierLotIds', ids);
        ...
        $scope.Valid = function () {
            ...
            $http({
                method: 'POST',
                url: config.url + '/CourrierLot/ValidRelance',
            ...
            .success(function (data, status, headers, config) {
                $scope.refreshGrid();
                $rootScope.$broadcast('CourrierLotValidated');
            })
